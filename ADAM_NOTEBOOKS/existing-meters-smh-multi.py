...
ENGINE = create_engine('postgresql://avnadmin:AVNS_zSrniBsHGVQSqhqunlJ@pg-unhcr-unhcr-007.a.aivencloud.com:15602/defaultdb', pool_size=20, max_overflow=0,
...
def save2db(data):
    for val in data:
        ...
        try:
            ...
            val[1].to_sql(f"{val[0]}", ENGINE, if_exists='replace')
        except Exception as e:
            print(f'SQL ERROR EEEEEEEEEEEEEEEEEE {e}')
...
for serial in sorted(L_SQL_SERIALS):
    ...
    try:
        ...
        if len(sql_pool) > MAX_POOL:
            poolst = time.perf_counter()
            print(f'starting pool {len(sql_pool)}')
            # with Pool() as pool:
            #     result = pool.map(save2db, sql_pool)
            #     sql_pool = []
            if __name__ == '__main__':
                _processes = []
                for query in sql_pool:
                    # .to_sql(f"{val[0]}", ENGINE, if_exists='replace')
                    _p = multiprocessing.Process(target=query[1].to_sql, args=(query[0],ENGINE, 'gb_2024', 'replace'))
                    _p.start()
                    _processes.append(_p)

                for _p in _processes:
                    _p.join()
            poolet = time.perf_counter()
            poolsecs = "{:.2f}".format(poolet-poolst)
            poolmins = "{:.2f}".format((poolet-poolst)/60)
            print("POOL finished in {} seconds - using multiprocessing".format(poolsecs))
            print("POOL finished in {} mins - using multiprocessing".format(poolmins))
            print("---------------------------")
        else:
            sql_pool.append([serial,copy.deepcopy(df_updated)])
            continue
