...
def meter_response(serial, timestamp):
    ...
    print('EyeDro Endpoint and Key Set', serial, timestamp)
    # Set URL with serial and timestamp,
    meter_url = "https://api.eyedro.com/customcmd?Cmd=Unhcr.GetData&DeviceSerial=" + str(serial) + "&DateStartSecUtc=" + str(timestamp) + "&DateNumSteps="+str(int(96*1))+f"&UserKey={USER_KEY_GET_DATA}"
    #get devices
...
##tz = 'Africa/Algiers'
days = 70
csv_name = "Bamyan_2022-04-17.csv"
...
while site_idx < end_idx:
    ...
    #     print(m,m["id"])
        ...
        while loop > 0:
            print(loop)
            meterDataWh = meter_response(m["id"], ts2Epoch(date.isoformat(), 4.5))
            meterDataWh = meterDataWh['Data']['Wh'][0]
            print(len(meterDataWh), '\n',meterDataWh[0],'\n')
            data.append(meterDataWh)
            loop -= 1
