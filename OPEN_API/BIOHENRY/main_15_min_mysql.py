...
def getData(report_data, date, tz, offset_hrs):
    ...
    dtEnd = '%s 23:59' % date.isoformat()[0:10]
    report_url = report_data["url"] % (dtStart, dtEnd)
    key = report_data["key"]
    fn = 'OPEN_API/BIOHENRY/data/galooli/%s_%s_liters_%s.csv' % (dtStart[0:10], report_data["site"], key)
    ...
    cookies = {
        "Token": "hub_eyJhbGciOiJBMjU2S1ciLCJlbmMiOiJBMjU2Q0JDLUhTNTEyIiwidHlwIjoiSldUIn0.TttqSCnb1FdZyRcAzSIizhXR6dNbQfVbMyN38MAYwIjlwJl15aKFeS00Q5CeW6DVh_kdMCQiUlmtZUAW4Xnvh6_H0n-8n_I7._JyD1zmbKmEk9EMU58-seg.rx58p87rIEReT3tp75prY3IowOZPh0N1yUS4UROkgkgDd25jIUNGscRnhzD8jqX2wQSorOj7V_buOV02BaGYziHjPfTwC6txuB6kSDxzkhgZyPKL_hJrU5jsVLSADim-DwHPabyb6iMLPAiG9jj_G4Abxa1XyA2QeTNl5fivgireIL0NZRSgJooStSFja-quD0cIp1Gp3cLS0MkP0VDfFQHrcMspZkA9NjNcbgrM3fodLmEGXFtGiFKynswPTG_1nRC4YdCSVDD82jxaU0tnKMk0t5SIWvP7OMUThQE6Cpk5taMC5Z6o2aPcv_2ariPjaPX3p3t1ZW6Qosa_bWSJOA.qGN0UanxTqJBfmsxQ06TfLj77k-QTo02HOxhy_26aYY"
    }
    ...
    if response.text:
        ...
        requests.get(CancelReportUrl + x.ReportUID, headers=headers, cookies=cookies)
        print('XXXXXXXX',x)
            ...
            for z in val:
                ...
                if tdelta == t_delta_minute:
                    lastData = {"ts":formatted_datetime, "end":formatted_datetime_end, "epoch": epoch,"l1":ll1, "l2":ll2, "dl1": l1-ll1, "dl2": l2-ll2,
                                    "hr1": h1, "hr2": h2, "dhr1": hr1-h1,  "dhr2": hr2-h2, "key":key}
    ...
    with open(fn, 'w', newline='') as f:
        writer = csv.writer(f)
        # don't write header
        ##writer.writerow(["ts", "end", "wh", "l1", "l2", "dl1", "dl2", "hr1", "hr2", "dhr1", "dhr2", "key"])
        for x in liters:
...
report_data = [
    {"site": "ABUJA", "meter_id": "00980AA5", "key":"ABUJA_OFFICE_DG1_and_DG2", "url": "https://space-fleet.galooli.com/Fleet/ExecuteFavoriteReport?objId=7214084&objType=u&startTime=%s&endTime=%s&favoriteId=10588"},
    {"site": "ADIKPO", "key":"ADIKPO_", "url": "https://space-fleet.galooli.com/Fleet/ExecuteFavoriteReport?objId=7214687&objType=u&startTime=%s&endTime=%s&favoriteId=10588"},
    {"site": "CALABAR", "key":"CALABAR_BASE_TANK", "url": "https://space-fleet.galooli.com/Fleet/ExecuteFavoriteReport?objId=7214680&objType=u&startTime=%s&endTime=%s&favoriteId=10588"},
    {"site": "LAGOS", "key":"UNHCR_LAGOS_OFFICE_DG1_and_DG2", "url": "https://space-fleet.galooli.com/Fleet/ExecuteFavoriteReport?objId=7214694&objType=u&startTime=%s&endTime=%s&favoriteId=10588"},
    {"site": "OGOJA", "key":"UNHCR_OGOJA_OFFICE_DG1_and_DG2_", "url": "https://space-fleet.galooli.com/Fleet/ExecuteFavoriteReport?objId=7214695&objType=u&startTime=%s&endTime=%s&favoriteId=10588"},
    {"site": "OGOJA", "key":"OGOJA_GUEST_HOUSE_", "url": "https://space-fleet.galooli.com/Fleet/ExecuteFavoriteReport?objId=7214015&objType=u&startTime=%s&endTime=%s&favoriteId=10588"},
    {"site": "TARABA", "key":"TARABA_DG1_And_DG2", "url": "https://space-fleet.galooli.com/Fleet/ExecuteFavoriteReport?objId=7214697&objType=u&startTime=%s&endTime=%s&favoriteId=10588"},
]
...
while site_idx < len(report_data):
    ...
    while loop > 0:
        ...
        liters[0] += ',123456'
        print(len(liters), liters[0],'\n', liters[0].split(','))
        int(liters[0].split(',')[2]) + offset_hrs *60*60
        #######################################
        # https://unhcr.eyedro.com/#toolsexporter
        meterDataWh = meter_response(report_data[site_idx]['meter_id'], ts2Epoch(dtStart))
        print('MMMMM',dtStart,'MMMMM')
        meterDataWh = meterDataWh['Data']['Wh'][0]
        print(fn, "\n",len(meterDataWh), meterDataWh[0:4])
        print(datetime.datetime.utcfromtimestamp(meterDataWh[0][0]))
